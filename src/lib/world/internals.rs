use crate::{
    ...
    data::{
        chunk::{ChunkData, ChunkTask, ChunkThread, NeedsRemesh},
        chunk_map::{ChunkMap, ChunkMapInsertBuffer, ChunkMapRemoveBuffer, ChunkMapUpdateBuffer},
    },
    materials::block::LoadingTexture,
    prelude::*,
    rendering::mesh_cache::{MeshCache, MeshCacheInsertBuffer, MeshRef},
    world_plugin::BlockWorldMaterialHandle,
};
use bevy::{
...
pub struct CameraInfo<'w, 's, C: BlockWorldConfig>(
    Query<'w, 's, (&'static Camera, &'static GlobalTransform), With<BlockWorldCamera<C>>>,
);

#[derive(Resource, Deref, DerefMut, Clone)]
pub struct ModifiedBlocks<C, I>(
    #[deref] Arc<RwLock<HashMap<IVec3, WorldBlock<I>>>>,
    PhantomData<C>,
);

impl<C: BlockWorldConfig> Default for ModifiedBlocks<C, C::MaterialIndex> {
    fn default() -> Self {
        Self(Arc::new(RwLock::new(HashMap::new())), PhantomData)
    }
}

impl<C: BlockWorldConfig> ModifiedBlocks<C, C::MaterialIndex> {
    pub fn get_block(&self, position: &IVec3) -> Option<WorldBlock<C::MaterialIndex>> {
        let modified_block = self.0.read().unwrap();
        modified_block.get(position).cloned()
    }
}

#[derive(Resource, Deref, DerefMut, Default)]
pub struct BlockWriteBuffer<C, I>(#[deref] Vec<(IVec3, WorldBlock<I>)>, PhantomData<C>);
    ...
    C: BlockWorldConfig,
{
    pub fn setup(mut commands: Commands, configuration: Res<C>) {
        ...
        commands.init_resource::<ModifiedBlocks<C, C::MaterialIndex>>();
        commands.init_resource::<BlockWriteBuffer<C, C::MaterialIndex>>();
    ...
    pub fn spawn_chunks(
        ...
        while let Some(chunk_position) = chunks_deque.pop_front() {
            ...
            if configuration.chunk_spawn_strategy() != ChunkSpawnStrategy::Near {
                continue;
            }
    ...
    pub fn retire_chunks(
        ...
        let chunks_to_remove = {
            ...
            for (chunk, view_visibility) in all_chunks.iter() {
                ...
                let distance_squared = chunk.position.distance_squared(chunk_at_camera);
                let near_camera = distance_squared
                    <= (CHUNK_SIZE_I * configuration.min_despawn_distance() as i32).pow(2);
                if (should_be_culled && !near_camera)
                    || distance_squared > spawning_distance_squared + 1
                {
                    remove.push(chunk);
                }
            }
    ...
    pub fn remesh_dirty_chunks(
        ...
        modified_blocks: Res<ModifiedBlocks<C, C::MaterialIndex>>,
        configuration: Res<C>,
    ) {
        let thread_pool = AsyncComputeTaskPool::get();

        for chunk in dirty_chunks.iter() {
            let block_data_fn = (configuration.block_lookup_delegate())(chunk.position);
            ...
            let thread = thread_pool.spawn(async move {
                chunk_task.generate(block_data_fn);
                ...
                let mesh_cache_hit = mesh_map
                    .read()
                    .unwrap()
                    .contains_key(&chunk_task.blocks_hash());
                if !mesh_cache_hit {
                ...
                if !chunk_task.is_full() {
                    let mesh_handle = {
                        if let Some(mesh_handle) =
                            mesh_cache.get_mesh_handle(&chunk_task.blocks_hash())
                        {
                            if let Some(user_bundle) =
                                mesh_cache.get_user_bundle(&chunk_task.blocks_hash())
                            {
                            ...
                            let hash = chunk_task.blocks_hash();
    ...
    pub fn flush_block_write_buffer(
        mut commands: Commands,
        mut buffer: ResMut<BlockWriteBuffer<C, C::MaterialIndex>>,
        mut ev_chunk_will_update: EventWriter<ChunkWillUpdate<C>>,
        chunk_map: Res<ChunkMap<C, C::MaterialIndex>>,
        modified_blocks: Res<ModifiedBlocks<C, C::MaterialIndex>>,
    ) {
        let chunk_map_read_lock = chunk_map.get_read_lock();
        let mut modified_blocks = modified_blocks.write().unwrap();

        let mut updated_chunks = HashSet::<(Entity, IVec3)>::new();

        for (position, block) in buffer.iter() {
            let (chunk_pos, _block_pos) = get_chunk_block_position(*position);
            modified_blocks.insert(*position, *block);
    ...
    pub(crate) fn assign_material<M: Material>(
        ...
        material_handle: Option<Res<BlockWorldMaterialHandle<M>>>,
    ) {
