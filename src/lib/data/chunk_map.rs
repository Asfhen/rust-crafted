use bevy::{
    ecs::{event::EventWriter, resource::Resource},
    math::{bounding::Aabb3d, IVec3, Isometry3d, Vec3, Vec3A},
...
use crate::{custom_meshing::CHUNK_SIZE_F, data::chunk::{self, ChunkData}, prelude::BLOCK_SIZE, world::world::ChunkWillSpawn};
...
impl<C: Send + Sync + 'static, I: Copy> ChunkMap<C, I> {
    ...
    pub fn get_world_bounds(read_lock: &RwLockReadGuard<ChunkMapData<I>>) -> Aabb3d {
        ...
        world_bounds.min *= CHUNK_SIZE_F * BLOCK_SIZE;
        world_bounds.max = (world_bounds.max + Vec3A::ONE) * CHUNK_SIZE_F * BLOCK_SIZE;
        world_bounds
    }
